...
class JSONExportManager:
    def __init__(self, config):
        self.config = config
        self.project_name = config.get("PROJECT", "MIGRATION_PROJECT")

    def transform_and_save(self, structured_data):
        """Builds the nested JSON structure from raw parser steps."""
        output = {
            "project_name": self.project_name,
            "extraction_timestamp": None, # Could add here
            "steps": []
        }
        ...
        for step in structured_data:
            ...
            step_entry = {
                ...
                "step_name": info.get('label'),
                "program_name": params.get('PGM'),
                "proc_name": params.get('PROC'),
                "parameters": params.get('PARM'),
                "cond_logic": params.get('COND'),
                "data_allocations": []
            }
            ...
            for dd in step['dds']:
                ...
                allocation_entry = {
                    ...
                    "unit": dd_params.get('UNIT'),
                    "vol_ser": dd_params.get('VOL'),
                    "is_dummy": bool(dd_params.get('DUMMY')),
                    "instream_payload": "\n".join(dd.get('payload', [])),
                    "technical_attributes": {
                        "lrecl": lrecl,
                        "blksize": blksize,
                        "recfm": recfm,
                        "extra": extra_dcb_attrs
                    }
                }
                step_entry["data_allocations"].append(allocation_entry)
            
            output["steps"].append(step_entry)

        filename = f"{self.project_name.lower()}_extract.json"
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump(output, f, indent=2)
        
        print(f"Extraction complete. JSON saved to: {filename}")
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            if path:
                ...
                exporter = JSONExportManager(config)
                exporter.transform_and_save(data)
            else:
